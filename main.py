...
def protein2emb_encoder(x, words2idx_p, max_p, model_params):  # 蛋白质编码，x是蛋白质序列
    if model_params['encode_method'] == 'smiles':
        t1 = x
    # elif model_params['encode_method'] == 'selfies':
    #   t1 = x   # 这里不一样
...
class BIN_Data_Encoder(data.Dataset):  
    ...
    def __getitem__(self, index):
        ...
        p_line = self.df.iloc[index]['Protein_embed']
        d_n = np.asarray(literal_eval(d_line))
...
def training_testing_process(config, options, model_params):  # options
        ...
        net = CrossAttention(model_params, feature_size=model_params["feature_size"], n_heads=config["n_heads"], d_dim=config["d_dim"], pooling_dropout=model_params["pooling_dropout"], linear_dropout=model_params["linear_dropout"])
        device ="cpu"
        ...
        with open(model_params['out_path'] + "/cv_result.txt", 'w') as f:
            ...
            for epoch in range(model_params["max_epoch"]):
                ...
                for i, (drug_id, protein_id,drug_mat, protein_mat, attn_mask_drug, attn_mask_protein, labels, drug_net, protein_net) in enumerate(validation_generator):
                    with torch.no_grad():
        ...
        with open(model_params['out_path'] + "/test_result.txt", 'w') as f:  
            ...
            net.eval()
            test_probs, test_labels , drug_ids, protein_ids = [], [],[],[]

            for i, (drug_id, protein_id,drug_mat, protein_mat, attn_mask_drug, attn_mask_protein, labels, drug_net, protein_net) in enumerate(test_generator):
                with torch.no_grad():    
                    probs = net(drug_mat.long().to(device), protein_mat.long().to(device),
                                attn_mask_drug.long().to(device), attn_mask_protein.long().to(device),
                                drug_net.to(device), protein_net.to(device))
                    ...
                    drug_ids.extend(list(drug_id))
                    protein_ids.extend(list(protein_id))
        ...
        return final_val_probs, final_val_labels, test_probs, test_labels, drug_ids, protein_ids
...
if __name__ == '__main__':
    ...
    model_params['out_path'] = './result' + '/result_%s' % args.DB + '/CA_P'
    ...
    bpe_codes_protein = codecs.open(vocab_path)  # codecs专门用作编码转换, 打开文件
    ...
    for iter_cal in range(iter_num):
        val_prob, val_label, test_prob, test_label, drug_ids, protein_ids = training_testing_process(config, options, model_params)
        ...
        output.to_csv(model_params['out_path'] + '/val_roc_%s.csv' %index_value[iter_cal], header=True, index=False)

        score_test = measure_evaluation(score_test, test_prob, test_label, iter_cal+1)
        output = pd.DataFrame([ drug_ids, protein_ids, test_prob, test_label], index=["drug_id", "protein_id","prob", "label"]).transpose()
        output.to_csv(model_params['out_path'] + '/test_roc_%s.csv' %index_value[iter_cal], header=True, index=False)
